...
#define LISTEN_QUEUE_DEPTH 3 // max client number to connect
...
int main()
{
    ...
    while (1)
    {
        ...
        int cliSocket = accept(serverSockFd, (struct sockaddr *)&clientSockAddr, (socklen_t *)&clientAddrLen);
        if (cliSocket == -1)
        {
        ...
        int res = getpeername(cliSocket, (struct sockaddr *)&addr, &addr_size);
        char clientip[20] = {0};
        strcpy(clientip, inet_ntoa(addr.sin_addr));

        printf("Connection accepted for IP=%s\n", clientip);
        ...
        if (state == 0)
        {
            ...
            // receive from client
            int rcvSize = recv(cliSocket, inBuf, sizeof(inBuf), 0);
            if (rcvSize == -1)
            {
        ...
        {
            // send some data back
            char *serverAnswer = "Server answer";
            int sentSize = send(cliSocket, serverAnswer, strlen(serverAnswer), 0);
            if (sentSize == -1)
            {
